...
COMMAND_LINK_PATH="/usr/local/bin/fp"
...
install_command_link() {
    ...
    if [ ! -f "$current_script" ]; then
        warn "未找到当前脚本文件，跳过 fp 命令安装。"
        return 0
    fi

    ensure_parent_dir "$COMMAND_LINK_PATH"
    cp "$current_script" "$COMMAND_LINK_PATH" || {
        warn "安装 fp 命令失败。"
        return 1
    }
    chmod +x "$COMMAND_LINK_PATH" || true
    info "已安装快捷命令: fp"
}
